...
function calculate() {
	...
	if (grouped) {
		...
		echo('result <- dlply(' + dataframe + ', ".groups", function(df) shapiro.test(df[["' + variableName + '"]]))\n');
	} else {
		echo('result <- shapiro.test(' + variable + ')\n');
	}
}
...
function printout() {
  ...
  if (filtered) {
	...
	if (grouped) {
		...
		echo(i18n("Statistic") + ' = result[[i]][[1]],');
